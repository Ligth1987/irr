...
Функционал: БО
  ...
  Сценарий: Создание компании
    Когда я вхожу на БО используя логин "nice" и пароль "nice911"
        * на БО я открываю страницу "Компании - Компании"
       То открыта страница списка компаний
        * я удаляю все компании с именем "Новая компания"

    Когда я создаю новую компанию со следующими параметрами:
         | поле              | значение                                  |
         | Название компании | Новая компания                            |
         | Активность        | true                                      |
         | Статус видимости  | true                                      |
         | Рубрика           | Все рубрики                               |
         | Регион            | Страны → Россия → Белгородская → Белгород |
         | Пакет             | все 10 (тест)                             |
         | Тип компании      | Лизинговая компания                       |
       То компания с именем "Новая компания" отображена в конце списка
    Когда открыта главная страница
        * открыт список объявлений для категории "Лизинг"
        * на странице фильтров я делаю поиск по параметрам:
          | поле   | значение |
          | Регион | Белгород |
       То на странице результатов показан ровно 1 дилер
        * у первого дилера "Название" равно "Новая компания"
        * у первого дилера "Количество объявлений" равно "0"
